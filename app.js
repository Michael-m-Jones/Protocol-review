...
const LS = {
  ...
  sessionReviewed: "proto_session_reviewed_v1",
};
...
function loadJSON(key, fallback){
  try{
    // session-only storage for sessionReviewed
    if(key === LS.sessionReviewed){
...
function saveSet(key, setObj){
  saveJSON(key, Array.from(setObj));
}

function markReviewedDrug(drug){
  const setObj = loadSet(LS.sessionReviewed);
  setObj.add(normalize(drug));
  saveSet(LS.sessionReviewed, setObj);
...
function updateSessionProgress(){
  ...
  if(!el) return;
  const total = new Set(ALL.map(c=>normalize(c.drug))).size;
  const done = loadSet(LS.sessionReviewed).size;
  el.textContent = `Session: ${done}/${total} drugs`;
}

function buildDrugIndex(){
  const map = new Map();
  ALL.forEach(c=>{
    const k = normalize(c.drug);
    if(!map.has(k)) map.set(k, {drug: c.drug, count: 0});
    map.get(k).count += 1;
  });
  return Array.from(map.values()).sort((a,b)=>a.drug.localeCompare(b.drug));
}

function renderDrugList(){
  const listEl = $("#drugList");
  if(!listEl) return;
  const filter = normalize($("#drugFilter")?.value || "");
  const items = buildDrugIndex().filter(x => !filter || normalize(x.drug).includes(filter));
  const currentDrug = FILTERED.length ? FILTERED[CARD_IDX].drug : "";
  listEl.innerHTML = items.map(x=>{
    const active = normalize(x.drug) === normalize(currentDrug);
    return `<div class="drug-item ${active ? "active":""}" data-drug="${escapeHTML(x.drug)}">
      <div>
        <div class="drug-name">${escapeHTML(x.drug)}</div>
        <div class="drug-meta">${x.count} card${x.count===1?"":"s"}</div>
      </div>
      <div class="drug-meta">›</div>
    </div>`;
  }).join("");

  listEl.querySelectorAll("[data-drug]").forEach(el=>{
    el.onclick = ()=>{
      const drug = el.getAttribute("data-drug");
      const idx = FILTERED.findIndex(c => normalize(c.drug) === normalize(drug));
      if(idx >= 0){
        CARD_IDX = idx;
        FLIPPED = false;
        render();
        ...
        searchTerm = "";
        applySearch();
        const idx2 = FILTERED.findIndex(c => normalize(c.drug) === normalize(drug));
        if(idx2 >= 0){
          CARD_IDX = idx2;
          FLIPPED = false;
          render();
...
function renderCards(){
  ...
  card.addEventListener("click", (e)=>{
    ...
    FLIPPED = !FLIPPED;
    if(FLIPPED) markReviewedDrug(c.drug);
    render();
  });
...
async function init(){
  ...
  $("#btnStartSession")?.addEventListener("click", ()=>{
    if(!confirm("Start a new session? This resets the session progress counter (missed/history stay).")) return;
    sessionStorage.removeItem(LS.sessionReviewed);
    updateSessionProgress();
    renderDrugList();
  });
  ...
  $("#btnReset").onclick = ()=>{
    ...
    localStorage.removeItem(LS.sessionReviewed);
    render();
  };
